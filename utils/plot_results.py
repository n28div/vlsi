...
experiments = {
    ...
    #"csv/sat/NaiveModel.csv": ("Naive model", "tab:blue"),
    #"csv/sat/SymmetryModel.csv": ("Symmetry breaking model", "tab:orange"),
    #"csv/sat/NaiveModelRot.csv": ("Naive model with rotations", "tab:green"),
    #"csv/sat/SymmetryModelRot.csv": ("Symmetry model with rotations", "tab:red"),
}
...
for filename, (title, color) in experiments.items():
    data = np.loadtxt(filename, skiprows=1, delimiter=",", converters={
        0: lambda x: re.search(r'([0-9]+)', x.decode())[0]
    })
    data[data[:, 3] < 0, 1] = 300
    data[data < 0] = np.NaN
